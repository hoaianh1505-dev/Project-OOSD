<?php
// Auth là authentication (xác thực)
// Authorization (phân quyền)
class AuthController
{
    // Chức năng đăng nhập hệ thống
    function login()
    {
        $email = $_POST['email'];
        $password = $_POST['password'];

        // 1. Kiểm tra tài khoản này có tồn tại trong hệ thống không?
        $customerRepository = new CustomerRepository();
        $customer = $customerRepository->findEmail($email);
        if (!$customer) {
            // nếu không có thì báo lỗi
            $_SESSION['error'] = "Email $email này không tồn tại trong hệ thống";
            // điều hướng về trang chủ
            header('location: index.php');
            exit;
        }

        // 2. Kiểm tra đúng mật khẩu không?
        if (!password_verify($password, $customer->getPassword())) {
            // nếu sai mật khẩu
            $_SESSION['error'] = "Sai mật khẩu";
            // điều hướng về trang chủ
            header('location: index.php');
            exit;
        }

        // 3. Kiểm tra xem tài khoản đã được kích hoạt chưa?
        if (!$customer->getIsActive()) {
            // nếu chưa được kích hoạt tài khoản
            $_SESSION['error'] = "Tài khoản chưa được kích hoạt, vui lòng vào email để kích hoạt tài khoản";
            // điều hướng về trang chủ
            header('location: index.php');
            exit;
        }

        // Đăng nhập thành công -> Lưu email và tên người dùng vào session
        $_SESSION['email'] = $email;
        $_SESSION['name'] = $customer->getName();

        if (!empty($_SESSION['return_url'])) {
            $return_url = $_SESSION['return_url'];
            unset($_SESSION['return_url']);
            header("location: $return_url");
            exit;
        }

        // Điều hướng về trang thông tin tài khoản
        header('location: ?c=customer&a=show');
    }

    // Chức năng đăng xuất
    function logout()
    {
        // Hủy tất cả các session
        session_destroy();
        header('location: index.php');
    }

    // Đăng nhập bằng Google OAuth
    function loginGoogle()
    {
        try {
            $clientID = GOOGLE_CLIENT_ID;
            $clientSecret = GOOGLE_CLIENT_SECRET;
            $redirectUri = get_domain() . $_SERVER['PHP_SELF'] . "?c=auth&a=loginGoogle";

            // Tạo Client Request để truy cập Google API
            $client = new Google_Client();
            $client->setClientId($clientID);
            $client->setClientSecret($clientSecret);
            $client->setRedirectUri($redirectUri);
            $client->addScope("email");
            $client->addScope("profile");

            if (isset($_GET['code'])) {
                $token = $client->fetchAccessTokenWithAuthCode($_GET['code']);
                $client->setAccessToken($token['access_token']);

                // Lấy thông tin profile thông qua token
                $google_oauth = new Google_Service_Oauth2($client);
                $google_account_info = $google_oauth->userinfo->get();
                $email =  $google_account_info->email;
                $name =  $google_account_info->name;
                $this->createCustomerBySocial($email, $name, "google");
                $this->setupLoginEnv($email, $name);
                header("location: index.php");
                exit;
            }
        } catch (Exception $e) {
            echo $e->getMessage();
        }
    }

    function createCustomerBySocial($email, $name, $type)
    {
        $customerRepository = new CustomerRepository();
        $customer = $customerRepository->findEmail($email);

        if (empty($customer)) {
            // Tạo tài khoản mới từ thông tin MXH gửi về
            $data = array(
                "name" => $name,
                "mobile" => "",
                "password" => "",
                "email" => $email,
                "shipping_name" => $name,
                "shipping_mobile" => "",
                "ward_id" => null,
                "housenumber_street" => null,
                "login_by" => $type,
                "is_active" => 1
            );
            $customerRepository->save($data);
        }
    }

    function setupLoginEnv($email, $name, $remember_me = null)
    {
        $_SESSION["email"] = $email;
        $_SESSION["name"] = $name;
    }

    // 🔴 ENDPOINT API DÍNH LỖI SQL INJECTION PHỤC VỤ BÁO CÁO LAB
    // Trả dữ liệu mảng thô (JSON) im lặng khi bị tấn công UNION, chuẩn thực tế 100%
    function verifyResetToken()
    {
        global $conn;
        
        $token = $_GET['token'] ?? '';
        $cid = $_GET['cid'] ?? '';

        // CHỦ ĐỘNG TẠO LỖI: Nối chuỗi trực tiếp tham số $cid và $token vào SQL của bảng customer thực tế
        $sql = "SELECT id, name, password, email 
                FROM customer 
                WHERE id = '" . $cid . "' AND name = '" . $token . "'";
        
        $result = $conn->query($sql);
        $dumpData = array();

        if ($result) {
            while ($row = $result->fetch_assoc()) {
                $dumpData[] = $row;
            }
        }

        // OUTPUT CHUẨN API NGOÀI ĐỜI: Im lặng in ra mảng dữ liệu thô, không chứa thông báo tự chế
        header('Content-Type: application/json; charset=utf-8');
        echo json_encode($dumpData, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
        exit;
    }
}